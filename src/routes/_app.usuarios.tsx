...
function UsuariosPage() {
  ...
  const toggleRole = async (userId: string, current: string) => {
    ...
    const { error } = await supabase.from("user_roles").insert({ user_id: userId, role: next as "admin" | "usuario" });
        ...
        <div className="divide-y divide-border">
          {users.data?.map((u: any) => (
            <div key={u.id} className="flex items-center gap-4 p-4">
              <div className={`grid h-10 w-10 place-items-center rounded-full ${u.role === "admin" ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}>
              ...
              <div className="text-xs text-muted-foreground hidden sm:block">{formatDate(u.created_at)}</div>
              {u.id !== auth.user?.id && (
