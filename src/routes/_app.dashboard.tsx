  ...
  return (
    <Card className="p-5">
      <div className="flex items-center justify-between">
        ...
        <div className={`grid h-9 w-9 place-items-center rounded-md ${accent ?? "bg-primary/10 text-primary"}`}>
  ...
  const stats = useQuery({
    ...
    queryFn: async () => {
      ...
      const [cli, doc, mes, recent] = await Promise.all([
        ...
        supabase.from("documentos").select("id", { count: "exact", head: true }).gte("created_at", since.toISOString()),
        supabase.from("documentos")
          .select("id, nome_original, tamanho, created_at, cliente:clientes(id, razao_social), categoria:categorias(nome)")
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard icon={FileText} label="Documentos" value={stats.data?.documentos ?? "—"} accent="bg-success/10 text-success" />
        <StatCard icon={CalendarDays} label="Enviados no mês" value={stats.data?.mes ?? "—"} accent="bg-warning/15 text-warning" />
        <StatCard icon={TrendingUp} label="Atividade" value={stats.data?.recent.length ?? "—"} accent="bg-accent text-accent-foreground" />
      </div>

      <Card className="mt-6 p-0 overflow-hidden">
        <div className="p-5 border-b border-border">
          <h2 className="font-display text-lg font-semibold">Últimos documentos</h2>
          <p className="text-sm text-muted-foreground">Os arquivos mais recentes adicionados ao sistema.</p>
        </div>
        <div className="divide-y divide-border">
          {(stats.data?.recent ?? []).length === 0 && (
            <div className="p-8 text-center text-sm text-muted-foreground">Nenhum documento ainda.</div>
          )}
          {stats.data?.recent.map((d: any) => (
            <Link key={d.id} to="/clientes/$id" params={{ id: d.cliente?.id ?? "" }} className="flex items-center gap-4 p-4 hover:bg-accent/40 transition-colors">
